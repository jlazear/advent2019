...
def command(program, pos, inpos, outputs, inputs, base=0):
    ...
    return program, pos, inpos, outputs, inputs, base, new_out


def intcode(program, noun=None, verb=None, inputs=None, outputs=None, copy_program=False,
            pos=None, base=None, inpos=None):
    if copy_program:
    ...
    inpos = 0 if (inpos is None) else inpos
    ...
    while pos < len(program):
        ...
        if program[pos] == 99:
            pos += 1
            halt = True
            return outputs, halt, waiting, new_out, pos, base, inpos
        else:
            try:
                program, pos, inpos, outputs, inputs, base, new_out = command(program, pos, inpos, outputs, inputs, base)
                if new_out: 
                    return outputs, halt, waiting, new_out, pos, base, inpos
            except IndexError:
                waiting = True
